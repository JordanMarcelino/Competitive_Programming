...
package kotlin_for_cp.hackerrank.electronic_shops
...
private fun solve(){
    val budget = stdin.int()
    val keyCount = stdin.int()
    val usbCount = stdin.int()

    val keyboards = IntArray(keyCount) { stdin.int() }
    val usbs = IntArray(usbCount) { stdin.int() }

    var max = -1

    for (keyboard in keyboards) {
        for (usb in usbs) {
            if (keyboard + usb <= budget) {
                max = max(max, keyboard + usb)
            }
        }
    }
