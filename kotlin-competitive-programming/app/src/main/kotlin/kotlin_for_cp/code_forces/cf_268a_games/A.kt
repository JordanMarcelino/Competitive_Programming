...
package kotlin_for_cp.code_forces.cf_268a_games
...
private fun solve() {
    ...
    val listPlayers: MutableList<Pair<Int, Int>> = mutableListOf()
    for (j in 0 until n) {
        val (a, b) = i.int() to i.int()
        listPlayers.add(a to b)
    }

    for(j in listPlayers.indices){
        for (k in listPlayers.indices){
            if(j != k){
                if(listPlayers[j].first == listPlayers[k].second){
                    ans++
                }
            }
        }
