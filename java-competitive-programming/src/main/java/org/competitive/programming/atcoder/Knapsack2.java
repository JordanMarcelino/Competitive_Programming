...
public class Knapsack2 {
    ...
    void solve() throws IOException {
        int n = i(), W = i();
        long[] w = readArrLong(n), v = readArrLong(n);

        out.println(knapSack(w, v, n - 1, W));
    }

    long[][] dp = new long[(int) 10e5][(int) 10e5];
    public long knapSack(long[] w, long[] v, int idx, long storage){
        if (idx == 0) return 0;
        if (dp[idx][(int) storage] != 0) return dp[idx][(int) storage];

        long best = knapSack(w, v, idx  - 1, storage);

        if (storage >= w[idx]){
            best = Math.max(best, v[idx] + knapSack(w, v, idx - 1, storage - w[idx]));
        }

        dp[idx][(int) storage] = best;
        return best;
    }
...
//            out.println("Case #" + i + ": ");
    ...
    public static void main(String[] args) throws IOException {
        Knapsack2 driver = new Knapsack2();
