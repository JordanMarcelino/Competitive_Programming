...
public class BetweenTwoSets {
    ...
    void solve() throws IOException {
        int N = i();
        int M = i();
        int[] factor = readArr(N);
        int[] factorOf = readArr(M);
        int ans = 0;

        sort(factor);
        sort(factorOf);

        for (int i = factor[factor.length - 1]; i <= factorOf[0]; i++) {
            boolean isFactor = true;

            for (int j = 0; j < factor.length; j++) {
                if (i % factor[j] != 0) isFactor = false;
            }

            for (int k = 0; k < factorOf.length; k++) {
                if (factorOf[k] % i != 0) isFactor = false;
            }
...
//            out.println("Case #" + i + ": ");
    ...
    public static void main(String[] args) throws IOException {
        BetweenTwoSets driver = new BetweenTwoSets();
