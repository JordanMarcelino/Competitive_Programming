...
public class CircularArrayRotation {
    ...
    void solve() throws IOException {
        ...
        int[] arr = readArr(N);
        int[] queries = readArr(Q);

        int[] last = Arrays.copyOfRange(arr, N - K, N);
        int[] first = Arrays.copyOfRange(arr, 0, N - K);
        int i = 0;

        for (int val : last) arr[i++] = val;
        for (int val : first) arr[i++] = val;
...
//            out.println("Case #" + i + ": ");
    ...
    public static void main(String[] args) throws IOException {
        CircularArrayRotation driver = new CircularArrayRotation();
